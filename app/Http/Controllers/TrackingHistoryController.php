...
{
    public function __construct(private readonly TrackingHistoryService $tracking) {}
    ...
    public function index(Request $request): View
    {
        ...
        return view('monitoring.tracking-history', [
            'pilgrims' => Pilgrim::query()
                ...
                ->orderBy('full_name')
                ->get(['id', 'branch_id', 'registration_number', 'full_name']),
        ]);
    }

    public function data(TrackingHistoryRequest $request): JsonResponse
    {
        $pilgrim = Pilgrim::with('branch:id,name')->findOrFail($request->integer('pilgrim_id'));

        return response()->json($this->tracking->history(
            $pilgrim,
